...
def uniswap_backtest_ETH(start_date,end_date,mini,maxi,target,hold_target):
    pool = "0x88e6a0c2ddd26feeb64f039a2c41296fcb3f5640" # ETH USDC Ethereum
    base = 0
    
    file_path = "graph" + ".csv"
    #get_data.update(file_path)    
...
def range_backtest(start_date,end_date,target_0,investment_method, my_strategy):
    if investment_method == "compound_interest":
        target = target_0
        df, buy, sell = my_strategy(filtering_data(start_date,end_date),start_date,end_date)
...
def analyze(df,target_0):
    ...
    print(f"annualized_std_dev{annualized_std_dev * 100 :.2f}%")
...
def show_analysis(important_sheets,target_0,fee_return,investment_method="compound_interest"):
    ...
    print(f"investment0 : {target_0}")
    #print(f"(diff of compare value) : {PNL}")
    print(f"impermanent_loss: {IL}")
    print(f"fee_return : {fee_return * target_0}")
    print(f"fee_return% : {fee_return*100:.2f}%")
    if investment_method == "compound_interest":
    ...
    print("===========================================")
    print("hold_strategy_value only")
